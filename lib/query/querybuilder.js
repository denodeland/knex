...
class Builder extends EventEmitter {
  ...
  where(column, operator, value) {
    ...
    if (argsLength === 3) {
      ...
      if (checkOperator === '<=>') {
        return this.whereNullSafe(column, value);
      }
  ...
  whereNot(column, ...args) {
    if (args.length >= 2) {
      if (args[0] === '<=>') {
        return this.whereRaw('NOT (?? <=> ?)', [column, args[1]]);
      }
  ...
  whereNullSafe(column, value) {
    ...
    return this.whereRaw(`?? ${operator} ?`, [column, value]);
  }

  orWhereNullSafe(column, value) {
    const operator = this._getNullSafeOperator();
    return this.orWhereRaw(`?? ${operator} ?`, [column, value]);
  }
