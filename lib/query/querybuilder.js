...
class Builder extends EventEmitter {
  ...
  whereNullSafe(column, value) {
    return this._whereNullSafe(this.where.bind(this), column, value);
  }

  orWhereNullSafe(column, value) {
    return this._whereNullSafe(this.orWhere.bind(this), column, value);
  }

  _whereNullSafe(method, column, value) {
    const operator = this._getNullSafeOperator();
    return method(column, operator, value);
  }

  _getNullSafeOperator() {
    const clientName = this.client.config.client;
    return clientName === 'mysql' || clientName === 'mysql2'
      ? '<=>'
